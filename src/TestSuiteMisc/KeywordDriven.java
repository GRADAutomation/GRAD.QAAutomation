...
public class KeywordDriven extends DriverScript{
	
	public static Properties APPTEXT;
	public static Excel_Ops testData;
	...
	public void initialize() throws IOException{
		...
		APPTEXT.load(fs);
		// intialize datatable
		controller= new Excel_Ops(System.getProperty("user.dir")+"\\src\\config\\Controller.xlsx");
		testData  =  new Excel_Ops(System.getProperty("user.dir")+"\\src\\config\\TestData.xlsx");
		...
		for(int tcid=2 ; tcid<=controller.getRowCount("Suite1");tcid++){
			...
			if(controller.getCellData("Suite1", "Runmode", tcid).equals("Y")){
				...
				for(int tsid=2;tsid<=controller.getRowCount(currentTest);tsid++){
					...
					ReportUtil.addStep(stepDescription, keyword, result, fileName);
				...
				// report skipped
				APPICATION_LOGS.debug("***********************************"+currentTest+" --- " +testStatus);
				//ReportUtil.addTestCase(currentTest, 										TestUtil.now("dd.MMMMM.yyyy hh.mm.ss aaa"),										TestUtil.now("dd.MMMMM.yyyy hh.mm.ss aaa"),										testStatus );
